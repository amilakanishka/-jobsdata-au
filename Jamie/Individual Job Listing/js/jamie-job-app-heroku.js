...
d3.csv(csvPath)
    ...
    d3.json(jsonPathJob)
        .then(data => {jobListing = data;
            ...
            // Filter to only visualise jobs with salary information provided
            jobListing = jobListing.filter(job => job.salary_min > 100);

            // Loop to show only minimum of top 50? (to be decided) or lenght of results 
            for (i = 0; i < Math.min(50,jobListing.length); i++) {
